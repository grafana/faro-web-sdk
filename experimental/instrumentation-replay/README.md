...
| ------------------ | ------------------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sanitizeMetaHref` | `boolean`                                         | `true`               | Redact query parameter values and fragments in `window.location.href` in rrweb Meta events before sending. Parameter keys are preserved; values are replaced with `**redacted**`. Prevents leaking OAuth codes, tokens, and PII from URLs. Only affects Meta events — URLs in DOM snapshots are not sanitized |
