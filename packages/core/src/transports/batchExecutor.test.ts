...
describe('BatchExecutor', () => {
  ...
  describe('config options', () => {
    ...
    it('trims the buffer to the item limit after a failed flush when the buffer is already full', () => {
      const itemLimit = 2;
      const item1 = generateTransportItem();
      const item2 = generateTransportItem();
      const reentrantItem = generateTransportItem();
      const dispatchError = new Error('dispatch failed');
      let be: BatchExecutor;
      ...
      be.addItem(item1);
      expect(() => be.addItem(item2)).toThrow(dispatchError);

      expect((be as unknown as { signalBuffer: TransportItem[] }).signalBuffer).toHaveLength(itemLimit);
