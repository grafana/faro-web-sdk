...
| `beforeSend`            | Hook to modify or discard an event before it is sent over the wire      | `(TransportItem) => TransportItem \| null` | Y        | `undefined`    |
| `globalObjectKey`       | String that should be used when defining the agent on the global object | `string`                                   | Y        | `grafanaAgent` |
| `ignoreErrors`          | Error message patterns for errors that should be ignored                | `Array<string \| RegExp>`                  | Y        | `[]`           |
