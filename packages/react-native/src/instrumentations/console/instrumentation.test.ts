...
describe('ConsoleInstrumentation', () => {
  ...
  it('Handles objects with circular references', () => {
    ...
    // Without serializeErrors, circular refs fallback to [object Object]
    expect((mockTransport.items[0] as TransportItem<ExceptionEvent>)?.payload.value).toBe(
  ...
  it('unpatch restores original console methods', () => {
    ...
    initializeFaroCore(
      mockConfig({
        transports: [mockTransport],
        instrumentations: [consoleInstrumentation],
        unpatchedConsole: {
          error: jest.fn(),
        } as unknown as Console,
      })
    ...
    console.error('after unpatch');
    // Should still be 1 item (not 2) because console is unpatched
    expect(mockTransport.items).toHaveLength(1);
  });
});
