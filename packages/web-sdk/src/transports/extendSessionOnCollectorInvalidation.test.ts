...
describe('extendSessionOnCollectorInvalidation', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(sessionManagerMock, 'getSessionManagerByConfig').mockReturnValue({
      fetchUserSession: jest.fn().mockReturnValue({ sessionId: 'stale-session' }),
  ...
  it('does not rotate when another tab already replaced the session in storage', () => {
    const config = mockConfig({
      sessionTracking: {
        enabled: true,
        persistent: true,
    ...
    jest.spyOn(sessionManagerMock, 'getSessionManagerByConfig').mockReturnValue({
      fetchUserSession: jest.fn().mockReturnValue({ sessionId: 'tab-b-session' }),
      storeUserSession: jest.fn(),
    } as unknown as ReturnType<typeof sessionManagerMock.getSessionManagerByConfig>);
